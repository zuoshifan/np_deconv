...
def np_rotate(hpmap, angle):
    """Rotate an input healpix map around the north pole.
    ...
    hpmap : float, array-like
        An array containing the map, supports masked maps.
    angle : float
        The rotation angle in degree, between 0 and 360.

    """
    out_map = rotate_map(hpmap, rot=(0, -90, 0))
    out_map = rotate_map(out_map, rot=(0, 0, angle))
    out_map = rotate_map(out_map, rot=(0, 90, 0))
